    ...
    let footerButtonsHTML = `
        <div style="display: flex; gap: 8px; align-items: center;">
            ${role === 'admin' && person && person.id ? `
                <button type="button" class="btn-admin btn-admin--danger btn-delete-person-from-db" title="Видалити з БД" style="width:32px; height:32px; padding:0; display:flex; align-items:center; justify-content:center;">${icon('trash', 14)}</button>
            ` : ''}
            ${(!currentUser || (role !== 'admin' && role !== 'moderator' && role !== 'editor')) ? `
                <input type="text" id="edit-person-propose-comment" class="admin-input" placeholder="Коментар до правки..." style="max-width: 260px; font-size: 12px; height: 32px; margin-bottom: 0;">
            ` : ''}
        </div>
        <div style="display: flex; gap: 8px; align-items: center;">
            <button type="button" class="btn-admin btn-admin--secondary btn-close-person-modal">Скасувати</button>
            ${(() => {
                if (role === 'admin') {
                    return `
                        <button type="button" class="btn-admin btn-admin--primary btn-admin--purple btn-save-person-direct">Записати в БД</button>
                        <button type="button" class="btn-admin btn-admin--primary btn-save-person-approve" style="background: var(--green);">Записати і прийняти</button>
                    `;
                    ...
                    return `
                        <button type="button" class="btn-admin btn-admin--primary btn-save-person-propose" style="background: var(--yellow);">Запропонувати</button>
                    `;
                }
            })()}
        </div>
