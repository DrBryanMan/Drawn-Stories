    ...
    async render() {
            ...
            <div class="ds-modal ds-modal--large" id="collection-editor-modal">
                ...
                <div class="ds-modal-footer" style="display:flex; justify-content:space-between; align-items:center; padding:16px 24px; border-top:1px solid var(--border-s);">
                    <div style="display:flex; gap:8px; align-items:center;">
                        ${currentUser && currentUser.role === 'admin' && this.collection && this.collection.id ? `
        ...
        if (deleteBtn) {
            deleteBtn.addEventListener('click', async () => {
                if (!confirm(`Ви дійсно бажаєте видалити збірку "${this.collection.name}" з бази даних?`)) return;
                try {
                    await API.delete(`/collections/${this.collection.id}`);
                    this.close();
